...
This repository contains our caching algorithm designed to optimize ontology instance retrieval by accelerating reasoners or concept learners. The implementation supports various eviction strategies and can be configured to use different cache types and sizes. Below, I have provided instructions for running the algorithm and generating the results.
...
### Caching the reasoners
 ...
 To run the EBR reasoner on the Family and Father datasets, with ratios 0.1, 0.4, and 0.8, on a cold cache (i.e., we initialize the cache with a small number of concepts), using the LRU eviction strategy, first make sure you are located in this directory, then run
